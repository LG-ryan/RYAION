...
class TradingViewAlert(BaseModel):
    """TradingView Webhook Alert 페이로드 (v5 Real Data)"""
    ts_unix: int = Field(..., description="Unix timestamp")
    symbol: str = Field(..., description="심볼 (SPX, AAPL 등)")
    timeframe: str = Field(..., description="타임프레임 (1W, 1D, 4H 등)")
    ...
    # v5 Real Data 추가 필드 (Optional)
    signal_number: Optional[int] = Field(None, description="신호 번호")
    sl_price: Optional[float] = Field(None, description="손절가")
    tp_price: Optional[float] = Field(None, description="익절가")
    atr: Optional[float] = Field(None, description="ATR 값")
    mode: Optional[str] = Field(None, description="데이터 소스 (yfinance, real_data, etc)")
    
    # Legacy 필드 (Optional for backward compatibility)
    prob: Optional[float] = Field(None, ge=0, le=1, description="확률 (0-1)")
    dist_ath: Optional[float] = Field(None, ge=0, description="ATH 대비 거리")
